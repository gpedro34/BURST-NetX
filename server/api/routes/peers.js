...
const peers = async (firstIndex, amount) => {
	...
	// get peers from DB
	const peers = await control.peers(null, null, amount, firstIndex);
	return peers;
};
...
const peersByPlatform = async (id, platform) => {
	...
	if (typeof plat !== 'undefined' && plat) {
		...
		try {
			...
			completePeers.sort((a, b) => a.id - b.id);
			return {
	...
	if (req.params.requestType) {
		switch (req.params.requestType) {
			...
			case 'getPeersByHeight':
				...
				if (!obj.overHeight) {
					obj = {
						error: 'We are yet to see a peer over height ' + req.params.height
					};
				}
				break;
			case 'getPeersById':
				...
				obj.peers = await peers(
					Number(req.params.start),
					Number(req.params.howMany)
				);
				break;
		}
		if (obj) {
			...
			const err = {
				error:
					'Something went wrong with DB call - Report Exception 30 at https://github.com/gpedro34/BURST-NetX/issues/new?assignees=&labels=&template=bug_report.md&title='
			};
			res.json(err);
			err.req = req;
			console.error(err);
		...
		const err = { error: 'Not a valid API call' };
		res.json(err);
		err.req = req;
		console.error(err);
		return;
