...
function start() {
	...
	xhr.open(
		'GET',
		apiDomain + 'peer?address=' + address + '&uptimeTimetable=30-days',
		true
	);
	xhr.onload = function(e) {
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
				...
				let date = new Date(response.discovered).toGMTString();
				console.log(date);
				...
				for (let a = 0; a < response.info.length; a++) {
					if (
						response.info[a].ip !== 'No IP' &&
						response.info[a].ip !== 'N/A'
					) {
						response.info[a].lastScanned = new Date(
							response.info[a].lastScanned
						).toGMTString();
						if (response.info[a].sslFrom !== 'N/A') {
							response.info[a].sslFrom = new Date(
								response.info[a].sslFrom
							).toGMTString();
						}
						if (response.info[a].sslTo !== 'N/A') {
							response.info[a].sslTo = new Date(
								response.info[a].sslTo
							).toGMTString();
						}
					}
					...
					document.getElementById('info').innerHTML +=
						...
						response.info[a].ip +
						'</td><td>' +
						response.info[a].apiPort +
						'</td><td>' +
						response.info[a].blocked +
						'</td><td>' +
						response.info[a].city +
						'</td><td>' +
						response.info[a].country +
						'</td><td>' +
						response.info[a].ssl +
						'</td><td>' +
						response.info[a].lastScanned +
